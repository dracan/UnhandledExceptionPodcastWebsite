...
**Reason**: The site no longer uses the m10c look-and-feel. Replaced by the redesign requirement below.

**Migration**: The hardcoded SCSS colour variables (`$darkest-color` and friends) and the m10c
component partials (`_app`, `_posts_list`, `_tag`, `_tags_list`, `_pagination`, `_icon`,
`_error_404`) are deleted. Colours are now CSS custom properties in `css/main.scss`.

### Requirement: Custom style overrides retained

**Reason**: The overrides described styling for markup the redesign removed. The listen-on
sprite is replaced by inline brand SVGs, the Buy Me a Coffee image button by a styled link, and
the custom tag-cloud classes by the sidebar tag chips.

**Migration**: `.listen-on-icon`, `#bmc-container`, `.tags-list-custom` and `#patreon-container`
