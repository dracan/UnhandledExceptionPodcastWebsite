---
title: "Episode 82: AI and the Microsoft Agent Framework - with James World"
date: 2025-12-17
episodeId: 18369719
tags: ["ai", "dotnet"]
twitter_cards: true
images: ["images/0082-JamesWorld/JamesWorld.jpg"]
permalink: "/posts/0082-jamesworld/"
layout: "layouts/post.njk"
---

In this episode, I was joined by [James World](https://www.linkedin.com/in/james-world-08a55a1/) to chat about AI for developers. We covered a lot of general AI chat - from using it to quickly knock up tools, to spec-driven development, and much more. We also discussed the [Microsoft Agent Framework](https://github.com/microsoft/agent-framework) - a new open-source framework for building AI agents and multi-agent workflows.

James World is a seasoned technology leader and Solution Architect with over three decades of experience building and guiding software solutions across the financial and enterprise sectors. He's a passionate polyglot programmer, with deep expertise in C# and the Azure ecosystem, including years spent learning and teaching at Microsoft where he was a consultant advising customers building solutions on the Microsoft stack. Outside of hands-on engineering and architecture, he loves exploring continuous learning, mentorship, and ways to help teams level up their craft - topics he's shared on The Unhandled Exception Podcast and in tech communities around .NET and cloud computing.

{% buzzsprout 18369719 %}

---

# James' social links

* [GitHub](https://github.com/james-world)
* [LinkedIn](https://www.linkedin.com/in/james-world-08a55a1/)
* [Twitter/X](https://x.com/jamesw0rld)
* [Blog](https://www.zerobugbuild.com/)

# Links from the show

* [Microsoft Agent Framework - GitHub](https://github.com/microsoft/agent-framework)
* [Microsoft Agent Framework - Documentation](https://learn.microsoft.com/en-gb/agent-framework/overview/agent-framework-overview)
* [GitHub Spec Kit](https://speckit.org/)
* [Semantic Kernel](https://learn.microsoft.com/en-us/semantic-kernel/overview/)
* [AutoGen](https://github.com/microsoft/autogen)
* [Episode 16 - Learning and Self Development with James World](/posts/0016-learningwithjamesworld/)
* [Episode 23 - Git - with Jesse Liberty and James World](/posts/0023-git/)
* [James' 99-line coding agent](https://gist.github.com/james-world/857409d64bccb6c4f992a406250361d1)
* [Twitter/X post from Dan's dev tip](https://x.com/SahilBloom/status/1990813045354815878)

---

If you're enjoying the podcast, please remember to subscribe and share this episode with your friends and colleagues!
